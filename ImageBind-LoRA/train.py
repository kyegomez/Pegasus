...
class ImageBindTrain(L.LightningModule):
    def __init__(
        ...
        lora_layer_idxs=None,
        lora_modality_names=None,
    ...
    def info_nce_loss(self, batch, mode="train"):
        ...
        for feats_idx, feats_tensor in enumerate(feats_tensors):
            ...
            # Logging loss
            self.log(
                mode + "_loss_" + contrast[feats_idx],
                nll,
                prog_bar=True,
                on_step=LOG_ON_STEP,
...
def parse_args():
    ...
    parser.add_argument(
        "--full_model_checkpoint_dir",
        type=str,
        default="./.checkpoints/full",
        help="Directory to save the full model checkpoints",
    )
    parser.add_argument(
        "--full_model_checkpointing",
        action="store_true",
        help="Save full model checkpoints",
    )
    parser.add_argument(
        "--loggers",
        type=str,
        nargs="+",
        choices=["tensorboard", "wandb", "comet", "mlflow"],
...
if __name__ == "__main__":
    ...
    for logger in args.loggers if args.loggers is not None else []:
        if logger == "wandb":
            ...
            wandb_logger = pl_loggers.WandbLogger(
                save_dir=args.loggers_dir, name="imagebind"
            )
    ...
    model = ImageBindTrain(
        ...
        momentum_betas=args.momentum_betas,
    ...
    if args.full_model_checkpointing:
        checkpointing = {
            ...
            "callbacks": [
                ModelCheckpoint(
                    monitor="val_loss",
                    dirpath=args.full_model_checkpoint_dir,
